...
//    }
    ...
    void Draw(vector<Enemy> &enemies, Hero hero, Map structers, sf::Sprite background)
    {
        ...
        for(int l = 0; l < Enemy::count; ++l) {
            if(enemies[l].it > 0) {
                handleBullets(enemies[l].Bullets, enemies, structers, enemies[l].prewTimeShoot, enemies[l].it);
        ...
        // drawing bullets
        if(itBullets > 0) {
            handleBullets(Bullets, enemies, structers, prevTimeShoot, itBullets);
    ...
    void handleBullets(vector<Bullet>& Bullets, vector<Enemy>& enemies, Map& objects, float& prevTimeShoot, int& itBullets)
        {
            time = clock.getElapsedTime();
            ...
            if (curTime1 - prevTimeShoot > renderTimeShoot)
            {
                ...
                for (int k = 0; k < itBullets; ++k) //Ïðîâåðêà ïóëè íà ïîïàäåíèå âî âðàãà
                {
                    for (int l = 0; l < Enemy::count; ++l)
                    {
                        ...
                        if (itBullets == k) //×òîáû íå ïðîèçîøëî âûõîäà çà ãðàíèöû âåêòîðà
                        {
                            break;
                        }
                    }
                ...
                for (int k = 0; k < itBullets; ++k) //Ïðîâåðêà ïóëè íà ïîïàäåíèå â ïðåïÿòñòâèå
                {
                    for (size_t i = 0; i < objects.obj.size(); ++i)
                    {
                        ...
                        if (itBullets == k) //×òîáû íå ïðîèçîøëî âûõîäà ïðå ãðàíèöû âåêòîîðà
                        {
                            break;
                        }
                    }
