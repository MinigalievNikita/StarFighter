...
//    }
    ...
    void Draw(vector<Enemy> &enemies)
    {
        ...
        for(int l = 0; l < Enemy::count; ++l) {
            ...
            if(enemies[l].it_Bullets > 0) {
                handleBullets(enemies[l].Bullets, prevTimeShoot1, enemies[l].it_Bullets);
    ...
    void handleBullets(vector<Bullet> &Bullets, float &prevTimeShoot, int &itBullets){
        time = clock.getElapsedTime();
